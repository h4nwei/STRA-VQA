...
class Trainer:
    def __init__(self, args, model, loader, loss):
        ...
        self.loader_train, self.loader_test = loader['train'], loader['test'] #validation
    ...
    def train(self, is_plot=False):
        ...
        for bi, (x, info, length, y, scale, dis_name, ref_name) in enumerate(self.loader_train, start=1):
            ...
            loss, loss_items = self.loss(input_mos=y_pred, target_mos=y)
            loss.backward()
            self.optimizer.step()
            # y_pred = y_pred.squeeze()
            y_pred = y_pred * scale

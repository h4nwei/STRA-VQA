...
class PLCCRankLoss(nn.Module):
    def __init__(self):
        super(PLCCRankLoss, self).__init__()
    ...
    def forward(self, y_pred, y):
        ...
        rank_result = (torch.sum(ranking_loss) / y_pred.shape[0] / (y_pred.shape[0] - 1) / scale)
        ...
        plcc_result = ((loss0 + loss1) / 2)
        return (plcc_result+0.1*rank_result)
